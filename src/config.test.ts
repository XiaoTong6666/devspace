...
assert.equal(loadConfig({ ...baseEnv, DEVSPACE_LOG_LEVEL: "debug" }).logging.format, "pretty");
...
writeFileSync(
  ...
  JSON.stringify({
    ...
    oauth: {
      allowedRedirectHosts: ["oauth.example.com"],
    },
    fileShare: {
...
assert.deepEqual(fileConfig.oauth.allowedRedirectHosts, ["oauth.example.com"]);
